...
| **Project dependency integrity** | The project's declared deps and lockfile | Any install other than the project's deterministic-from-lockfile install, a hand-placed artifact matching the pin included | **Every subagent, on its own initiative or under user direction** — only the main session may run one, and only under user direction |
