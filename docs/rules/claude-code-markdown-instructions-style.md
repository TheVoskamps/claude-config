...
A model loads an instruction file into its context whole, so each
line in it costs context in every session that loads it, whether or
not that session ever acts on it. The rules below weigh a line against
that cost.
...
`##` heading, or as a `###` heading under a `##` section.
