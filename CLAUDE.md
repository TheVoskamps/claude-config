...
docs/rules/git-recovery.md — read when a commit is rejected for missing
  signatures, a merge is requested, a rebase stops on a conflict, or a
  commit landed on the wrong branch. Kernel: never squash merge, and
  never merge the default branch into another — rebase the other
