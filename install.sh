...
# Self-contained installer for the global Claude config source repo.
#
...
#      `origin` pointing at the source repo) to `~/.claude/`. You can
...
#     the source repo, the script reports that and exits without
#     moving anything.
...
if [[ -z "${CLAUDE_INSTALL_REEXEC:-}" ]]; then
    ...
    # script lives at <clone>/install.sh, so the clone root is the
...
main() {
    ...
    # Idempotency: if ~/.claude is already a clone of the source repo,
    # there is nothing to install. Don't move anything; don't create a
    # backup.
    if claude_dir_is_our_clone; then
        echo
        success "$CLAUDE_DIR is already a clone of the source repo."
    ...
    if [[ -n "$backup" ]]; then
        ...
        echo "Note that '$CLAUDE_DIR' is now a clone of the source repo, which"
        echo "carries its own .gitignore. So:"
        echo "  - Overlaid files that the source repo TRACKS (i.e. are not"
    ...
    echo "$CLAUDE_DIR is now a live clone of the source repo; update it with:"
